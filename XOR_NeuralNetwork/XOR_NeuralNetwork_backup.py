...
#import tensorflow as tf   #jc
#model_path = "/tmp/model1.ckpt" #模型儲存路徑設定 #jc
...
class Layer:
    ...
    def __init__(self, dim, id, act, act_prime, isoutputLayer = False):
        self.weight = 2 * np.random.random(dim) - 1  #調權重
    ...
    def update(self, learning_rate, left_a): #更新權重 left_a
...
class NeuralNetwork:
    ...
    def __init__(self, layersDim, activation='tanh'): #激活(勵)函數的定義
    ...
    def fit(self, X, y, learning_rate=0.1, epochs=1000): #100000 jc 訓練幾個週期
...
# 載入 MNIST 資料庫的訓練資料，並自動分為『訓練組』及『測試組』
   ...
   #查看mnist資料集筆數
    ...
    nn.fit(X, y, learning_rate=0.1, epochs=5000) #5000000 jc
    ...
    print("Correct rate: ", format(float(Yes)/float(Yes+No),'.2f'))
